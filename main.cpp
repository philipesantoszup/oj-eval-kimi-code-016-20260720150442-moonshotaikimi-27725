...
const int MAX_KEYS = MAX_DEGREE - 1;  // Max 127 keys per node
...
public:
    ...
    void insert(const char* key, int value) {
        ...
        // If leaf is full, we must split first
        if (leaf.key_count >= MAX_KEYS) {
            split_leaf(leaf_id, leaf);
            
            // After split, find the correct leaf to insert
            leaf_id = find_leaf(key);
            read_node(leaf_id, leaf);
            
            // Check for duplicate again in new leaf
            for (int i = 0; i < leaf.key_count; i++) {
                if (strcmp(leaf.keys[i], key) == 0 && leaf.values[i] == value) {
                    return;
                }
        ...
        // Shift elements
        memmove(&leaf.keys[pos + 1], &leaf.keys[pos], (leaf.key_count - pos) * sizeof(leaf.keys[0]));
        memmove(&leaf.values[pos + 1], &leaf.values[pos], (leaf.key_count - pos) * sizeof(leaf.values[0]));
    ...
    void remove(const char* key, int value) {
        ...
        for (int i = 0; i < leaf.key_count; i++) {
            if (strcmp(leaf.keys[i], key) == 0 && leaf.values[i] == value) {
                memmove(&leaf.keys[i], &leaf.keys[i + 1], (leaf.key_count - i - 1) * sizeof(leaf.keys[0]));
                memmove(&leaf.values[i], &leaf.values[i + 1], (leaf.key_count - i - 1) * sizeof(leaf.values[0]));
                leaf.key_count--;
                write_node(leaf_id, leaf);
                return;
            }
    ...
    std::vector<int> find(const char* key) {
        ...
        // Check if we need to scan multiple leaves
        while (leaf_id != -1) {
            BPTNode leaf;
            read_node(leaf_id, leaf);
            ...
            if (found || (leaf.key_count > 0 && strcmp(leaf.keys[leaf.key_count - 1], key) < 0)) {
                // Continue to next leaf to find more or if all keys < search key
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (cmd == "insert") {
            ...
            std::string index;
            std::cin >> index;
            std::vector<int> result = tree.find(index.c_str());
