...
private:
    ...
    void insert_into_parent(int left_id, const char* key, int right_id, int parent_id) {
        ...
        // Re-read parent in case root_id changed
        read_node(parent_id, parent);
        
        if (parent.key_count > MAX_KEYS) {
            // Only split if parent is still the root or a valid internal node
            if (parent_id == root_id || parent.parent_id != -1) {
...
int main() {
    ...
    
    {
        ...
        for (int i = 1; i <= 10; i++) {
            ...
            if (result.size() != 1 || result[0] != i) {
                std::cout << "FAIL: key" << i << " expected " << i << " got " << (result.empty() ? -1 : result[0]) << std::endl;
            }
        }
