...
private:
    ...
    int find_leaf(const char* key) {
        ...
        while (true) {
            ...
            std::cout << "find_leaf: internal node " << node_id << ", going to child " << node.children[i] << std::endl;
    ...
    void split_leaf(int leaf_id, BPTNode& leaf) {
        ...
        std::cout << "  split_pos=" << split_pos << ", old_count=" << leaf.key_count << ", new_count=" << new_leaf.key_count << std::endl;
    ...
    void insert_into_parent(int left_id, const char* key, int right_id, int parent_id) {
        std::cout << "insert_into_parent: left_id=" << left_id << ", right_id=" << right_id << ", parent_id=" << parent_id << std::endl;
...
public:
    ...
    void insert(const char* key, int value) {
        ...
        std::cout << "  leaf_id=" << leaf_id << ", key_count=" << leaf.key_count << std::endl;
...
int main() {
    {
        ...
        // Insert keys to trigger multiple splits
        for (int i = 1; i <= 10; i++) {
            char key[64];
    ...
    {
        BPTree tree;
        std::vector<int> result = tree.find("key01");
        std::cout << "Result: ";
        for (int v : result) {
            std::cout << v << " ";
        }
